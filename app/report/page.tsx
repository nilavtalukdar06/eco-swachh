...
export default function Report() {
  ...
  const [verificationStatus, setVerificationStatus] = useState<
    "idle" | "verifying" | "success" | "faliure"
  >("idle");
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Remove any markdown code blocks (e.g. "```json" and "```")
      ...
      try {
        ...
        if (
          ...
          setVerificationStatus("faliure");
        }
      } catch (error) {
        console.error("Failed to parse to JSON responses", error);
        setVerificationStatus("faliure");
      }
    } catch (error) {
  ...
  return (
    <section className="p-4 md:p-8 max-w-6xl mx-auto overflow-x-hidden">
      ...
      <h2 className="text-3xl font-semibold mb-6 text-gray-800 max-[1000px]:hidden">
        Recent Reports
      </h2>
      <div className="w-full bg-white rounded-2xl shadow-lg border-2 overflow-hidden max-[1000px]:hidden">
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full">
            <thead className="bg-gray-50 sticky top-0">
              <tr>
                <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-[11px] sm:text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Location
                </th>
                <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-[11px] sm:text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Type
              ...
              {reports.map((report) => (
                ...
                >
                  <td className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                    <MapPin className="inline-block w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 text-green-500" />
                    <span className="align-middle">{report.location}</span>
                  </td>
                  <td className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                    {report.wasteType}
                  </td>
